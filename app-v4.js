...
function pickExercises(pool, count, state, used, status) {
  ...
  // sessionsSinceUsed returns Infinity for a movement never trained. Adding
  // the random tiebreak to Infinity leaves Infinity, so every unused exercise
  // scored identically and the sort comparator returned NaN, which collapsed
  // to the pool's original order. Result: the first few entries in each slot
  // got picked forever and everything below them was unreachable. Capping the
  // freshness score keeps "never used" ranked highest while letting the random
  // tiebreak actually shuffle among them.
  const FRESH_CAP = 99;
  const scored = eligible.map(ex => ({
    ex,
    score: Math.min(sessionsSinceUsed(state, ex.id), FRESH_CAP) + Math.random() * 1.5
